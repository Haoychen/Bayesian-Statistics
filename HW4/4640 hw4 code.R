...
log_a_over_b<-seq(3,7,length.out=100) 
log_b<-seq(-8,-1,length.out=100)
beta<-exp(log_b)
alpha = exp(log_a_over_b) * beta
I<-length(alpha); J<-length(beta)
log_posterior<-matrix(rep(NA,I*J),I,J)

fi = 0.001
for(i in 1:I){
  for(j in 1:J){
    fact = (alpha*beta)^(fi-1) * exp(-fi*(alpha+beta))
    posterior[i,j]<-fact*prod(beta[j]^alpha[i]/(beta(alpha[i],y)*(beta[j]+1)^(alpha[i]+y)))
  }
}
contour(log_a_over_b,log_b,posterior/max(posterior),drawlabels=F,xlab="log(alpha/beta)",ylab="log(beta)")
...
a.
x<-read.table("~/desktop/data/meta.asc.txt")
x<-x[,-1]
